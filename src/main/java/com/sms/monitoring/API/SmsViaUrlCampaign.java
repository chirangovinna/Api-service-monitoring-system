...
public class SmsViaUrlCampaign {
    String apiname = "Sms Via Url Campaign API ";
    private static final Logger logger = LoggerFactory.getLogger(SMSapi.class);

    private static final String API_URL = "https://e-sms.dialog.lk/api/v1/message-via-url/create/url-campaign";
    private static final String esmsqk = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6MTE5MjQsImlhdCI6MTY5MTEyMDM5NywiZXhwIjo0ODE1MzIyNzk3fQ.xsmRopTcUmz_AAxnhG_T00AmQvf7SRtcDcU29-6YCH0";
    String errorCode;

    public void sendMessageViaUrlCampaign(String numberList, String sourceAddress, String message, String pushNotificationUrl) throws IOException, URISyntaxException, InterruptedException {
        String requestId = Integer.toString(((int) java.time.Instant.now().getEpochSecond()))+ Integer.toString((new Random()).nextInt(100));
        
        String endpoint = API_URL + "?esmsqk=" + esmsqk + "&list=" + numberList + "&source_address=" + sourceAddress + "&message=" + message + "&push_notification_url=" + pushNotificationUrl;
